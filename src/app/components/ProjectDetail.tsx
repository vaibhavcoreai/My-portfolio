...
import { ArrowLeft, Type, Moon, Sun } from 'lucide-react';
  ...
  return (
    <motion.div 
      ...
      className={`min-h-screen transition-colors duration-300 ${
        darkMode ? 'bg-black text-white' : 'bg-[#f5f5f0] text-black'
      }`}
    >
      {/* Fixed Controls Bar */}
      <motion.div
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        className={`fixed top-0 left-0 right-0 z-50 border-b-2 transition-colors duration-300 ${
          darkMode ? 'bg-black border-white' : 'bg-[#f5f5f0] border-black'
        }`}
      >
      ...
      <div className="pt-24 pb-20 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <motion.div
            ...
            className={`aspect-[16/9] mb-12 overflow-hidden ${
              darkMode ? 'border-2 border-white' : ''
            }`}
          >
            <img
              src={project.image}
          ...
          >
            <div className={`flex gap-4 mb-6 ${currentSize.label}`}>
              <span className={`px-4 py-2 border-2 rounded-full tracking-[0.2em] uppercase ${
                darkMode ? 'border-white' : 'border-black'
              }`}>
                {project.category}
              </span>
            ...
            <p className={`${currentSize.body} leading-relaxed ${
          ...
          <motion.div
            ...
            className={`grid md:grid-cols-2 gap-8 mb-16 pb-16 border-b-2 ${
              darkMode ? 'border-white/20' : 'border-black/20'
            }`}
          >
            <div>
              <p className={`${currentSize.label} tracking-[0.2em] uppercase mb-2 ${
              ...
              {project.results.map((result, index) => (
                <motion.div
                  ...
                  className={`p-6 border-2 ${
                    darkMode ? 'border-white/40 bg-white/5' : 'border-black/20 bg-black/5'
                  }`}
                >
            ...
            <div className="flex flex-wrap gap-3">
              {project.technologies.map((tech, index) => (
                <motion.span
                  ...
                  className={`${currentSize.label} px-4 py-2 border-2 rounded-full tracking-[0.15em] uppercase ${
                    darkMode 
